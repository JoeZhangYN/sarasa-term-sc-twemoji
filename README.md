...
3. VS Code `settings.json`：
   ...
   ```
   > Sarasa Term 是终端紧凑字体（line/em = 1.25），未配 `lineHeight` 时多数编辑器会显得行距偏挤。1.5 / 1.3 是经验体感值，太松调小、太挤调大。
...
| 行距看着太挤 | Sarasa Term 本就紧凑（line/em = 1.25），编辑器默认按此渲染 | settings.json 加 `"editor.lineHeight": 1.5` / `"terminal.integrated.lineHeight": 1.3`（详见安装步骤 3） |

## Changelog

- v0.1.2（2026-04-25）：README 推荐 `editor.lineHeight: 1.5` / `terminal.integrated.lineHeight: 1.3`。Sarasa Term 自身 line/em = 1.25 设计偏紧，metric 对齐后绝对行距仍可能偏挤——这是字体设计取向，由编辑器层 lineHeight 调节。
